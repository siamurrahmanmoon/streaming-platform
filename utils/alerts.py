...
class AlertManager:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                download_timeout = aiohttp.ClientTimeout(
                    total=60, connect=10, sock_read=50
                )
                async with session.get(
                    photo_url, timeout=download_timeout
                ) as photo_response:
                    if photo_response.status != 200:
                        logger.error(
                            f"❌ Poster download failed: HTTP {photo_response.status}"
                        )
                        return False
                ...
                async with session.post(
                    telegram_url, data=form, timeout=download_timeout
                ) as resp:
                    if resp.status == 200:
                        logger.info("☑️ Telegram photo sent successfully!")
                        return True
    ...
    async def send_discord(self, message: str, color: int = 16711680) -> bool:
        if not self.discord_webhook or not self.discord_webhook.startswith(
            DISCORD_WEBHOOK_PREFIXES
        ):
            return False
        try:
    ...
    async def notify_video_upload_success(self, metadata) -> bool:
        ...
        if poster_url:
            ...
            logger.error(
                "❌ Required Telegram poster notification skipped: no poster URL."
            )
    ...
    async def notify_unmatched_video(self, filename: str, title: str, year: int = None):
        ...
        message += (
            f"💡 Rename the file according to the TMDB naming convention and try again."
        )
