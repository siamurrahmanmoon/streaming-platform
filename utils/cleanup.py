...
def cleanup_orphaned_files(folder_path: str) -> int:
    ...
    tqdm.write(f"🧹 Checking for orphaned files older than {config.ORPHAN_DAYS_LIMIT} days...")
            ...
            if Path(file).suffix.lower() in video_extensions:
                ...
                if file_age > limit_seconds:
                    ...
                    while target_path.exists():
                        target_path = quarantine_dir / f"{file_path.stem}_{counter}{file_path.suffix}"
                        counter += 1
