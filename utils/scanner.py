...
async def scan_and_upload(uploader, folder_path: str):
    """
        ...
        for file in files:
            if Path(file).suffix.lower() in video_extensions:
                ...
                if config.CHECK_DUPLICATE_IN_DB:
                    try:
                        result = uploader.supabase.table('videos').select('id').eq('file_path', file_path).execute()
    ...
    tqdm.write(f"⚡ Concurrent Upload Limit: {config.MAX_CONCURRENT_UPLOADS} files at a time\n")
    ...
    async def process_with_limit(file_path):
        async with semaphore:
            await uploader.upload_single_video(file_path)
