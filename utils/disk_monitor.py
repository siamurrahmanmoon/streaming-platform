...
class DiskMonitor:
    ...
    def __init__(
        self, warning_threshold: float = 90.0, critical_threshold: float = 95.0
    ):
    ...
    async def check_and_alert(self, path: str = "/") -> bool:
        ...
        try:
            ...
            if usage_percent >= self.critical_threshold:
                ...
                logger.critical(
                    f"🚨 CRITICAL: Disk usage at {usage_percent:.1f}% - Only {free_gb:.2f} GB free!"
                )
                return False
                ...
                logger.warning(
                    f"⚠️ WARNING: Disk usage at {usage_percent:.1f}% - {free_gb:.2f} GB free"
                )
                return True
            else:
                logger.debug(
                    f" Disk usage: {usage_percent:.1f}% ({free_gb:.2f} GB free)"
                )
                return True
