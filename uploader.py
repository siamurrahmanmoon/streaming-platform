...
class AnimeVideoUploader:
    def __init__(self):
        ...
        # DoodStream FTP
        self.dood_ftp_server = os.getenv('DOODSTREAM_FTP_SERVER', 'ftp.doodstream.com')
        self.dood_username = os.getenv('DOODSTREAM_USERNAME')
        self.dood_password = os.getenv('DOODSTREAM_PASSWORD')

        # MixDrop API
        self.mixdrop_api_url = os.getenv('MIXDROP_API_URL', 'https://ul.mixdrop.ag/api')
    ...
    def _validate_credentials(self):
        missing = []
        if not self.dood_username or not self.dood_password:
        ...
        if missing:
            tqdm.write(f"⚠️ Missing credentials for: {', '.join(missing)}")
    ...
    def parse_video_filename(self, filename: str) -> Optional[Dict]:
        ...
        language_map = {
            ...
            'bn': 'Bangla', 'bangla': 'Bangla', 'bengali': 'Bangla',
            ...
            'original': 'Original', 'jap': 'Original',
    ...
    async def upload_to_doodstream_ftp(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            async with aioftp.Client.context(
                self.dood_ftp_server,
                user=self.dood_username,
                password=self.dood_password
            ) as client:
                with tqdm(
                    total=file_size, unit='B', unit_scale=True,
                    desc=f"📤 DoodStream: {file_name[:40]}",
    ...
    async def upload_to_mixdrop(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            tqdm.write(f"❌ MixDrop upload timed out.")
            return None
        except Exception as e:
    ...
    # ═══════════════════════════════════════════════════════
    # MIXDROP REMOTE UPLOAD (Fallback)
    # ══════════════════════════════════════════════════════
    async def upload_to_mixdrop_remote(self, source_url: str, file_name: str) -> Optional[str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                params = {
                    'email': self.mixdrop_email,
                    'key': self.mixdrop_key,
                    'url': source_url
                }

                async with session.get(
                    'https://api.mixdrop.ag/remotedl/add',
                    params=params,
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                with open(file_path, 'rb') as f:
                    ...
                    with tqdm(
                        total=file_size, unit='B', unit_scale=True,
                        desc=f"📤 StreamTape: {file_name[:40]}",
                        file=sys.stdout, leave=False
                    ) as pbar:
                        async with session.post(
                            upload_url,
                ...
                # Fallback patterns
                patterns = [
                    r'(https://streamtape\.com/v/[a-zA-Z0-9]+/[^\s"<]+)',
                    r'v/([a-zA-Z0-9]+)/',
                ]
                ...
                for pattern in patterns:
                    ...
                    if match:
                        if match.group(0).startswith('http'):
                            ...
                            file_url = f"https://streamtape.com/v/{file_id}/{file_name}"
            ...
            tqdm.write(f"❌ StreamTape upload timeout")
            return None
        except Exception as e:
    ...
    # ══════════════════════════════════════════════════════
    # SINGLE VIDEO UPLOAD (with Chain Upload logic)
    # ═══════════════════════════════════════════════════════
    async def upload_single_video(self, file_path: str):
        ...
        tqdm.write(f"   Anime: {metadata.anime_name}")
        tqdm.write(f"   Episode: {metadata.episode_number} | Quality: {metadata.quality}")
        tqdm.write(f"   Languages: {', '.join(metadata.languages)}")
        tqdm.write(f"   Size: {file_size / (1024*1024):.2f} MB")
        tqdm.write(f"{'='*70}")

        # ─── PHASE 1: Concurrent Uploads ───────────────────
        # DoodStream + StreamTape + MixDrop Direct (all at once)
        dood_task = asyncio.create_task(self.upload_to_doodstream_ftp(file_path))
        streamtape_task = asyncio.create_task(self.upload_to_streamtape(file_path))
        mixdrop_direct_task = asyncio.create_task(self.upload_to_mixdrop(file_path))

        dood_url, streamtape_url, mixdrop_url = await asyncio.gather(
            dood_task, streamtape_task, mixdrop_direct_task,
            return_exceptions=True
        )

        metadata.doodstream_url = str(dood_url) if not isinstance(dood_url, Exception) and dood_url else ""
        metadata.streamtape_url = str(streamtape_url) if not isinstance(streamtape_url, Exception) and streamtape_url else ""
        metadata.mixdrop_url = str(mixdrop_url) if not isinstance(mixdrop_url, Exception) and mixdrop_url else ""

        # ─── PHASE 2: MixDrop Fallback (Chain Upload) ──────
        # If MixDrop direct failed but StreamTape succeeded, use remote upload
        if not metadata.mixdrop_url and metadata.streamtape_url:
            tqdm.write(f"\n🔄 MixDrop direct failed. Trying Remote Upload from StreamTape...")
            mixdrop_remote_url = await self.upload_to_mixdrop_remote(
                metadata.streamtape_url, file_name
            )
        ...
        uploaded_count = sum([
            1 for url in [metadata.doodstream_url, metadata.mixdrop_url, metadata.streamtape_url]
            if url
        ])
        ...
        # Save to Supabase
        self.save_to_supabase(metadata)
        tqdm.write(f"✅ Database updated: {metadata.status}")

        # Print URLs
        if metadata.doodstream_url:
            tqdm.write(f"   🎬 DoodStream: {metadata.doodstream_url}")
        if metadata.streamtape_url:
            tqdm.write(f"   🎬 StreamTape: {metadata.streamtape_url}")
        if metadata.mixdrop_url:
            tqdm.write(f"   🎬 MixDrop: {metadata.mixdrop_url}")
